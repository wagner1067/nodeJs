  ...
  return new Promise((resolve, reject) => {
    fs.writeFile(filePath, content, "utf8", (err) => {
      if (err) {
        ...
        resolve(`File '${filePath}' written successfully`); //Resolve a Promise indicando sucesso
      }
    });
  });
}
  ...
  return new Promise((resolve, reject) => {
    fs.unlink(filePath, (err) => {
      if (err) {
        ...
        resolve(`File '${filePath}' deleted successfully`); //Resolve a Promise indicando sucesso
      }
    });
  });
}
